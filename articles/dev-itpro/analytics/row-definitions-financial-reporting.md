...
- 财务数据的链接，可以在 Microsoft Dynamics 365 for Finance and Operations 中包含维度值
...
3. 在**化整调整**对话框中，输入下列值：
    ...
    - **调整金额限制** – 通过自动调整指定限制的正整数。 此金额将与实际舍入差异的绝对值进行比较。
...
> [!NOTE]
> 根据为报表定义指定的**舍入精度**选项来应用调整限制。 例如，如果您选择将报表舍入到千分位并在**调整金额限制**字段中输入 **2**，则当**舍入调整行**字段中标识的值增加或减少 2,000 以上时，您将收到一条警告消息。
