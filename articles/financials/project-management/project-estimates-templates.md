...
必须添加筛选器以移除包含空交易记录类别的所有行。 无论使用默认模板还是创建自己的模板，都需要执行此任务。 此筛选器移除来自 Project Service Automation 且可能导致 Finance and Operations 中的工时预测不正确的所有汇总行。 选择**高级查询和筛选**链接以筛选掉 **msdyn\_transactioncategory\_value** 列中的空记录。
