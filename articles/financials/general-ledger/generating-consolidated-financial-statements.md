...
- 使用行定义中的**舍入调整**页，如下图中所示。

    ![货币折算调整舍入调整](./media/Currency-translation-adjustment-rounding-adjustments.png "货币折算调整舍入调整")

    如果指定应显示舍入调整 (CTA) 的行、资产总计行、总负债和权益行，以及所需阈值，财务报告将计算差并放入所需行中。 将创建一个名称为**舍入调整**的行，该行在向下钻取时显示，如下图中所示。

    ![舍入调整向下钻取](./media/rounding-adjustment-drill-down.png "舍入调整向下钻取")

- 将从资产到费用的所有科目放到一个区域内。 如下图中所示，差为与舍入调整 (CTA) 相同的金额。 因此，可将其用作检查总额以确保舍入调整页中不包含缺少的任何科目余额。

    ![舍入调整窗体检查](./media/rounding-adjustment-form-check.png "舍入调整窗体检查")
